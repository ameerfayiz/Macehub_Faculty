...
public class Current_Staffs extends Fragment {
    ...
    class listAdapter extends BaseAdapter {
        ...
        public int getCount() {
            return Current_Staffs.this.staffslist.size();
        }

        public View getView(final int i, View view, ViewGroup viewGroup) {
            view = LayoutInflater.from(Current_Staffs.this.getContext()).inflate(R.layout.itemlayout, null);
            ...
            link.setText(Current_Staffs.this.staffslist.get(i).getDepartment());

            ImageView imageView = (ImageView) view.findViewById(R.id.pic);
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("http://www.mace.ac.in/");
            stringBuilder.append(((staff) Current_Staffs.this.staffslist.get(i)).getImgloc());
            Glide.with(Current_Staffs.this.getContext()).load(stringBuilder.toString()).thumbnail(0.3f).transition(DrawableTransitionOptions.withCrossFade()).diskCacheStrategy(DiskCacheStrategy.ALL).override(160, 192).into(imageView);
            textView2.setText(((staff) Current_Staffs.this.staffslist.get(i)).getName());
            textView.setText(((staff) Current_Staffs.this.staffslist.get(i)).getDetails());
            if (((staff) Current_Staffs.this.staffslist.get(i)).getFav() == 1) {
                button.setBackground(Current_Staffs.this.getResources().getDrawable(R.drawable.ic_on));
            } else {
            ...
            ((Button) view.findViewById(R.id.call)).setOnClickListener(new OnClickListener() {
                public void onClick(View view) {
                    ...
                    Builder builder = new Builder(Current_Staffs.this.getContext());
                    ...
                    if (arrayList.size() > 0) {
                        builder.setItems((CharSequence[]) arrayList.toArray(new CharSequence[arrayList.size()]), new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialogInterface, int i) {
                                ...
                                Current_Staffs.this.startActivity(intent);
                            }
                        });
            ...
            button.setOnClickListener(new OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (((staff) Current_Staffs.this.staffslist.get(i)).getFav() == 0) {
                        ((staff) Current_Staffs.this.staffslist.get(i)).setFav(1);
                        databaseHandler.setfav(1, ((staff) Current_Staffs.this.staffslist.get(i)).getId());
                        databaseHandler.close();
                        button.setBackground(Current_Staffs.this.getResources().getDrawable(R.drawable.ic_on));
                        Toast.makeText(Current_Staffs.this.getActivity(), "Made Favourite", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ((staff) Current_Staffs.this.staffslist.get(i)).setFav(0);
                    databaseHandler.setfav(0, ((staff) Current_Staffs.this.staffslist.get(i)).getId());
                    databaseHandler.close();
                    button.setBackground(Current_Staffs.this.getResources().getDrawable(R.drawable.ic_off));
                    Toast.makeText(Current_Staffs.this.getActivity(), "Deleted From Favourites", Toast.LENGTH_SHORT).show();
                }
            });
            return view;
        }
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        this.staffslist = databaseHandler.getStaffsByDep((String) this.depcodes.get(i));
        if (this.staffslist.size() <= 0) {
            return LayoutInflater.from(getContext()).inflate(R.layout.itemmissinglayout, null);
        }
        View inflate = layoutInflater.inflate(R.layout.fragment_current__staffs, viewGroup, false);
        this.list = (ListView) inflate.findViewById(R.id.cur_list);
